...
{
    public class srvMarcarTransferidoController : ApiController
    {
        ...
        public srvMarcarTransferidoController(ILogger logger, IProspectProcessor prospectProcessor)
        {
            _logger = logger;
            _prospectProcessor = prospectProcessor;
        }

        public HttpResponseMessage MarcarTransferido(MarcaTransferido obj)
        {
            try
            {
                if (obj == null)
                {
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "el objeto NotificaOportunidadTransferencia no puede ir vacio");
                }

                return Request.CreateResponse<bool>(HttpStatusCode.OK, _prospectProcessor.MarcarTransferido(obj));
            }
