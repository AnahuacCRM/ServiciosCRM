...
{
    public class srvCambioRechazadoController : ApiController
    {
        ...
        public srvCambioRechazadoController(ILogger logger, IProspectProcessor prospectProcessor)
        {
            _logger = logger;
        ...
        [HttpPost]
        public HttpResponseMessage ActualizaRechazado(Rechazado rechazado)
        {
            try
            {
                if (rechazado == null)
                {
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "el objeto Rechazado no puede ir vacio");
                }
