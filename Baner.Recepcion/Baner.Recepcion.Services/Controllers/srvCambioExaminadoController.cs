...
namespace Baner.Recepcion.Services.Controllers
{
    ...
    /// Servicio 3 CRM.
    /// </summary>
    public class srvCambioExaminadoController : ApiController
    {
        private readonly ILogger _logger;
        private readonly IProspectProcessor _prospectProcessor;

        public srvCambioExaminadoController(ILogger logger, IProspectProcessor prospectProcessor)
        {
            _logger = logger;
        ...
        [HttpPost]
        public HttpResponseMessage ActualizaAExaminado(Examinado examinado)
        {
            try
            {
                if (examinado == null)
                {
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "El objeto Examinado no puede ir vacio");
                }

                return Request.CreateResponse<bool>(HttpStatusCode.OK, _prospectProcessor.UpdateExaminado(examinado));
            }
