...
{
    [TestClass]
    public class WebApiEstatusAlumnoTest
    {
        ...
        UpdateEstatusAlumnoController _updateestatusalumnoontroller;
        Token token;
        #endregion
        ...
        public void Initialize()
        {
           ...
           // serverconnection = new ServerConnection();
            ...
            _updateestatusalumnoontroller = new UpdateEstatusAlumnoController(_logger, _prospectprocessor);

            _updateestatusalumnoontroller.Request = new HttpRequestMessage()
            {
        ...
        [TestMethod]
        public void URLEstatusAlumnoSucessTest()
        {
            //Arrange
            var url = "http://localhost:21292/Api/UpdateEstatusAlumno";
            HttpClient proxy = new HttpClient();
            proxy.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
            //Act
            string json = JsonConvert.SerializeObject(estatusalumno);
            var response = proxy.PostAsJsonAsync(url, estatusalumno).Result;
        ...
        [TestMethod]
        public void EstatusAlumnoControllerSuccessTest()
        {
            //Arrange


            //Act
            var resultado = _updateestatusalumnoontroller.Post(estatusalumno);
