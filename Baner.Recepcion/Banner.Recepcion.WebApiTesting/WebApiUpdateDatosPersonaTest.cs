...
{
    [TestClass]
    public class WebApiUpdateDatosPersonaTest
    {
        ...
        UpdateDatosPersonalesController _updateDatosPersonalesController;
        Token token;
        #endregion
        ...
        public void Initialize()
        {
            ...
            _updateDatosPersonalesController = new UpdateDatosPersonalesController(_logger, _prospectprocessor);

            _updateDatosPersonalesController.Request = new HttpRequestMessage()
            {
                Properties = { { HttpPropertyKeys.HttpConfigurationKey, new HttpConfiguration() } }

            };

            #region Informacion de Datos Personales
            datospersona = new DatosPersona()
            {
                IdBanner = "00294811",
                Apellidos = "VEGA*BETANCOURT",
                Nombre = "ERICK",
                SegundoNombre = "ARMANDO",
                FechaNacimiento = new CustomDate() { Year = 2019, Month = 1, Day = 2 },
                Nacionalidad = "AL",
                EstadoCivil = "S",
                Religion = "CA",
                Sexo = "M"
        ...
        [TestMethod]
        public void URLDatosPersonaSucessTest()
        {
            //Arrange
            var url = "http://localhost:21292/Api/UpdateDatosPersonales";
            HttpClient proxy = new HttpClient();
            proxy.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
            //Act
            string json = JsonConvert.SerializeObject(datospersona);
            var response = proxy.PostAsJsonAsync(url, datospersona).Result;
        ...
        [TestMethod]
        public void DatosPersonaControllerSuccessTest()
        {
            //Arrange


            //Act
            var resultado = _updateDatosPersonalesController.Post(datospersona);
