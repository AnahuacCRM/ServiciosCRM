...
{
    [TestClass]
    public class WebApiNuevoIngresoTest
    {
        #region Propiedades
        ILogger _logger;
        ...
        UpdateNuevoIngresoController _updateNuevoIngresoController;
        Token token;
        #endregion
        ...
        public void Initialize()
        {
           ...
           // serverconnection = new ServerConnection();
            ...
            _updateNuevoIngresoController = new UpdateNuevoIngresoController(_logger, _prospectprocessor);

            _updateNuevoIngresoController.Request = new HttpRequestMessage()
            {
                Properties = { { HttpPropertyKeys.HttpConfigurationKey, new HttpConfiguration() } }

            };

            #region Informacion de Nuevo Ingreos

            nuevoingreso = new NuevoIngreso()
            {
                OportunidadIdCRM = new Guid("11a0f943-5ced-e611-8109-e0071b6ad151"),
                IdBanner = "00315500",
                Periodo = "201710",
                FechaSeleccionCursos = new CustomDate() { Year = 2016, Month =12, Day = 21 },
                VPDI="UAN"
            };
            #endregion
        ...
        [TestMethod]
        public void URLNuevoIngresoSucessTest()
        {
            //Arrange
            var url = "http://localhost:21292/Api/UpdateNuevoIngreso";
            HttpClient proxy = new HttpClient();
            proxy.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
            //Act
            string json = JsonConvert.SerializeObject(nuevoingreso);
            var response = proxy.PostAsJsonAsync(url, nuevoingreso).Result;
        ...
        [TestMethod]
        public void NuevoIngresoControlerSuccessTest()
        {
            //Arrange
            ...
            //Act
            var resultado = _updateNuevoIngresoController.Post(nuevoingreso);
        ...
        [TestMethod]
        public void NuevoIngresoIDBANNERInexistenteSuccessTest()
        {
            //Arrange

            nuevoingreso.IdBanner = "123";
            //Act
            var resultado = _updateNuevoIngresoController.Post(nuevoingreso);
